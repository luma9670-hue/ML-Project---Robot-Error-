...
Due to this I spent a large amount of time playing with threshold values, seeing which values were optimizing for the confusion matrices. After talking to the LLM, I was able to understand the changes in the hyperparameters, but was not able to get a very effecitve model for the failures.
...
The tuned model catches roughly **45% more grip failures** and **44% more protective stops** that the baseline was missing entirely. The trade-off is lower precision, or more false alarms, but in this context a false alarm means an unnecessary check, while a missed failure means undetected damage or injury risk. The tuned model reflects that asymmetry intentionally.
...
![Feature Importance](v7features.png)
